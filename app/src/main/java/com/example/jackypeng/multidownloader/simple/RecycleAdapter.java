...
import com.example.jackypeng.multidownloader.service.DownloadService;
import com.example.jackypeng.multidownloader.utils.FileUtil;
...
public class RecycleAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecycleAdapter(List<MyBusinessInfo> myBusinessInfos, Context mContext) {
        ...
        void initData(final MyBusinessInfo businessInfo) {
            ...
            bt_action.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    //如果没有下载记录，则创建一条新的记录
                    if (downloadInfo == null) {
                        File d = new File(MultiDownloadApplication.getContext().getExternalCacheDir().getAbsolutePath(), "download");
                        if (!d.exists()) {
                            d.mkdirs();
                        }
                    ...
                    downloadInfo.setListener(new DataListener() {
                        @Override
                        public void onRefresh() {
//                          Log.i(TAG, "itemHolder:" + itemHolder.toString());
                            refresh();
                        }
                    });
                    //根据downloadInfo确定要执行的任务
                    switch (downloadInfo.getStatus()) {
                        case DownloadInfo.INIT:
